...
fn problem_2_stack_overflow() {
    ...
    println!("SmallStruct size: {}B (safe)", std::mem::size_of::<SmallStruct>());
    println!("MediumStruct size: {}B (risky)", std::mem::size_of::<MediumStruct>());
    println!("DangerousStruct size: {}B ⚠️ (overflow)", std::mem::size_of::<DangerousStruct>());
...
fn problem_3_rcu_mutex_pattern() {
    ...
    {
        let data = mutex.lock();
        println!("\n  Value: {} (requires Mutex lock - performance hit)", data.frequently_read);
    }
