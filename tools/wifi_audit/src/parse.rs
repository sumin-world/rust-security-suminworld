...
#[derive(Debug, Clone)]
pub enum Event {
    Beacon { bssid: [u8;6], ssid: Cow<'static, str>, chan: Option<u8> },
    ProbeReq { sta: [u8;6], ssid: Option<String> },
    ProbeRes { bssid: [u8;6], ssid: Cow<'static, str> },
}
...
pub fn parse_wifi_frame(mut bytes: &[u8]) -> Option<Event> {
    ...
    if bytes[0] == 0 && bytes[1] == 0 {
        ...
        if rt_len >= 8 && rt_len < bytes.len() { bytes = &bytes[rt_len..]; }
    }
...
fn parse_beacon(bytes: &[u8]) -> Option<Event> {
    ...
    while pos + 2 <= bytes.len() {
        ...
        match id {
            0 => { ssid = Some(String::from_utf8_lossy(&bytes[pos..pos+len]).to_string()); }
            3 => { if len == 1 { chan = Some(bytes[pos]); } }
            _ => {}
        }
        pos += len;
    }

    Some(Event::Beacon { bssid, ssid: ssid.unwrap_or_default().into(), chan })
}
...
fn parse_probe_req(bytes: &[u8]) -> Option<Event> {
    ...
    while pos + 2 <= bytes.len() {
        ...
        pos += 2;
        if pos + len > bytes.len() { break; }
        if id == 0 { ssid = Some(String::from_utf8_lossy(&bytes[pos..pos+len]).to_string()); }
        pos += len;
    }
...
fn parse_probe_res(bytes: &[u8]) -> Option<Event> {
    ...
    Some(Event::ProbeRes { bssid, ssid: ssid.unwrap_or_default().into() })
}
